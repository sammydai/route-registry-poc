...
public class AppGroupRouteProcessor implements Processor{
	@Override
	public List<DirectoryComponent> process(String routeRule, Contract contract, ProcessorChain processorChain) {
		...
		String appgroupName = routeRuleArrays[2] + "." + routeRuleArrays[3];
		...
		if (patternResult.size() == 0) {
			System.out.println("上一层级没有匹配到,appgroup结束责任链传递");
		} else {
			...
			while (iterator.hasNext()) {
				...
				if (childrenMap.containsKey(appgroupName)) {
					DirectoryComponent patternServiceDir = childrenMap.get(appgroupName);
					iterator.remove();
